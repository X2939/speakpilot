...
Demo 视频链接：[SpeakPilot AI 英语口语陪练系统](https://www.bilibili.com/video/BV1Q1Ex6GEBh/?share_source=copy_web&vd_source=b1f7d66b977e001365d02bd7a1806dbf)
...
- 练习风格：强提示、标准、少提示，适配不同开口和对话引导需求
- 连续语音输入：基于浏览器 Web Speech API，支持持续监听和自动续听，Chrome/Edge 效果最好
- 语音状态提示：展示正在监听、自动续听、权限异常等状态
- 对话发音评估：用户用语音回答时，系统基于识别文本和 ASR 置信度同步评估发音清晰度
- 表达复练：根据本轮更自然表达生成复练目标，用户可再次朗读并查看匹配度
- 文本备用输入：麦克风不可用时仍可完整演示
- AI 主动开场：进入场景后自动发起第一句真实对话
- AI 角色回复：后端支持 OpenAI 兼容接口
- 即时纠错：输出总分、流利度、准确性、词汇评分；语音回答时补充本轮发音清晰度、识别文本和置信度
- 课后总结：输出能力画像、错误统计、优势、主要问题、下一步目标和复练任务
- 历史摘要：生成课后总结后自动保存最近 20 次练习，可回看摘要、分数和原始对话
...
如果不填写 `OPENAI_API_KEY`，系统会自动进入本地兜底模式，保证基础练习流程可用。
...
单轮口语练习接口。请求包含场景、练习风格、用户输入和历史对话，返回 AI 角色回复、即时评分、纠错建议和教学提示。
...
- `localStorage`：保存最近 20 次历史练习摘要，包含场景、分数、总结、对话和反馈。
...
- 增加发音音素级评分
- 保存历史练习报告，形成学习曲线
- 将历史记录从浏览器本地存储迁移到 SQLite，支持跨设备和长期学习曲线
- 按 CEFR 等级和用户表现动态调整练习风格
- 增加老师端或班级管理视图
